...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = authError instanceof FirebaseError ? authError.message : "登録に失敗しました";
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-1 items-center justify-center px-4">
        <div className="card-surface w-full max-w-md rounded-3xl border border-white/60 bg-white/85 px-8 py-10 shadow-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Create account</p>
          <h1 className="mt-3 text-3xl font-semibold text-slate-900">新規登録</h1>
          <p className="mt-2 text-sm text-slate-500">
            メール・パスワード・団体名を登録すると、分析結果をクラウドに保存して再利用できます。
          </p>
